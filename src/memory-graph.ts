...
export async function findRelevantMemories(
  ...
  const _inputKeywordSet = new Set(inputKeywords);
  const now = Date.now();
...
export function formatMemoryContext(memories: MemoryNode[]): string {
  ...
  for (const memory of memories) {
    ...
    lines.push(
      `**Winner:** ${memory.winner} | **Consensus:** ${(memory.consensusScore * 100).toFixed(0)}% | **Confidence:** ${(memory.confidenceScore * 100).toFixed(0)}%`,
    );
